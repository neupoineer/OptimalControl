...
namespace OptimalControl.Forms
{
    ...
    /// 应用程序全局异常信息用户界面
    /// </summary>
    internal partial class frmGlobalException : Form
    {
        ...
        public frmGlobalException(Exception globalException, string applicationName, string developerName)
        {
            // 设计器构造控件所须方法
            InitializeComponent();
            ...
            // 如果日志文件存放目录不存在则重建它
            if (!Directory.Exists(@".\cache\ErrorLog"))
            {
                // 其它方法
                // DirectoryInfo myDi = new DirectoryInfo(@".\ErrLog");
                // myDi.Create();
                Directory.CreateDirectory(@".\cache\ErrorLog");
            }
            ...
            {
                // 关闭写入器
                if (mySw != null) mySw.Close();
                // 关闭文件流
                if (myFs != null) myFs.Close();
            }
        } 
        #endregion
        ...
        private void OpenDirectory(string path)
        {
            ...
            {
                MessageBox.Show("抱歉！未能在系统中找到指定的文件 'Explorer.exe'。", "Explorer.exe", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            // 显示发送邮件窗体
            frmSendFeedbackEMail frmSendFeedbackEMail = new frmSendFeedbackEMail();
            frmSendFeedbackEMail.ShowDialog();
        } 
        #endregion
    }
}